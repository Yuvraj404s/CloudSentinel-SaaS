...
public interface BillingRecordRepository extends JpaRepository<BillingRecord, Long> {
    ...
    @Query(value = "SELECT service_name, SUM(cost) FROM billing_records WHERE user_id = :userId AND EXTRACT(MONTH FROM billing_date) = :month AND EXTRACT(YEAR FROM billing_date) = :year GROUP BY service_name ORDER BY SUM(cost) DESC", nativeQuery = true)
